...
static void test_latency_report_uses_32_byte_service_blocks(void) {
    ...
    assert(pdu[2] == 44u && pdu[3] == 0u);
    assert(pdu[8] == 1u && pdu[9] == 0u);
    assert(pdu[40] == 25u && pdu[41] == 0u && pdu[42] == 0u && pdu[43] == 0u);
}

int main(void) {
    test_metrics_label_tlv();
    test_aes_gcm_aad_rejects_tamper();
    test_latency_report_uses_32_byte_service_blocks();
    return 0;
}
