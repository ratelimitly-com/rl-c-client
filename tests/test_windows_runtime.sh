...
grep -Fq 'OPENSSL_LIBDIR ?=' "$ROOT/examples/win32/Makefile" \
  || { echo "test_windows_runtime: Win32 build cannot select lib or lib64" >&2; exit 1; }
grep -Fq -- '-lgdi32 -lcrypt32' "$ROOT/examples/win32/Makefile" \
  || { echo "test_windows_runtime: static OpenSSL dependencies are missing" >&2; exit 1; }
