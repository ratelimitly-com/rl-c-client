...
static void send_result(pending_request_t *pending, int status, bool allowed) {
    ...
    request->res.status = allowed ? 200 : 429;
    request->res.reason = allowed ? "OK" : "Too Many Requests";
...
static void on_accept(h2o_socket_t *listener, const char *error) {
    ...
    if (socket) {
        h2o_accept(&global_app->accept, socket);
    }
}
...
int main(void) {
    ...
    if (create_listener(&app) != 0 || create_udp_watchers(&app) != 0) {
        fprintf(stderr, "failed to initialize H2O sockets: %s\n", strerror(errno));
        return EXIT_FAILURE;
    }
    ...
    while (!stop_requested && h2o_evloop_run(loop, 1000) == 0) {
    }
